...
public class Regina extends PiesaSah {
    public static boolean validq(int i1, int j1, int i2, int j2, PiesaSah[][] tabla) {
        if (((i1 == i2 || j1 == j2) && !(i1 == i2 && j1 == j2)) || ((Math.abs(i1 - i2) == Math.abs(j1 - j2))))
            if (checkpathq(i1, j1, i2, j2, tabla)) {
                if (tabla[i1][j1].getNumePiesa().charAt(0) == 'Q')
    ...
    public static boolean checkpathq(int i1, int j1, int i2, int j2, PiesaSah[][] tabla) {
        if (i1 < i2) {
            if (j1 < j2) {
    ...
    public void mutaPiesa(int x2, int y2, PiesaSah[][] tabla) {
        if (validq(x1, y1, x2, y2, tabla) == true) {
