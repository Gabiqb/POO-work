...
public class Nebun extends PiesaSah {
    public static boolean validn(int i1, int j1, int i2, int j2, PiesaSah[][] tabla) {
        if (Math.abs(i1 - i2) == Math.abs(j1 - j2))
            if (checkpathn(i1, j1, i2, j2, tabla)) {
                if (tabla[i1][j1].getNumePiesa().charAt(0) == 'N')
    ...
    public static boolean checkpathn(int i1, int j1, int i2, int j2, PiesaSah[][] tabla) {
        if (i1 < i2) {
            if (j1 < j2) {
                for (int i = 1; i < Math.abs(i1 - i2); i++)
                    if (tabla[i1 + i][j1 + i].getNumePiesa().charAt(0) != ' ')
                        return false;
                return true;
            }
            if (j1 > j2) {
    ...
    public void mutaPiesa(int x2, int y2, PiesaSah[][] tabla) {
        if (validn(x1, y1, x2, y2, tabla) == true) {
